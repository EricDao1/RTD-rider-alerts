...
class GsonRequestTest2 {
    ...
    fun parseNetworkResponseTest() {
        ...
        val header = Header("header1","value1")
        val header2 = Header("header2","value2")
        val headerList = ArrayList<Header>()

        headerList.add(header)
        headerList.add(header2)
        val response = NetworkResponse(200,responseData.toByteArray(),true,1000,headerList)
        ...
        val gsonRequest = GsonRequest("abcde", RTDAlertData::class.java,null,requestListener,errorListener)
        val response2 = gsonRequest.implementParseNetworkResponse(response)
